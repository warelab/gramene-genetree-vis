...
function calculateAlignmentFromConsensus(node) {
  ...
  var consensus = node.model.consensus;
  var deltaDepth = {};
  // find first non-zero frequency
  var i = 0;
  while (consensus.frequency[i] === 0) {
    i++;
  ...
  for (var j = i + 1; j < consensus.consensusLength; j++) {
...
function calculateAlignment(node) {
  ...
  if (node.model.cigar) alignments[nodeId] = cigarToHistogram(node.model.cigar);else if (node.children.length === 1) {
    // this happens when one child has no genomes of interest
    ...
    node.children.forEach(function (childNode) {
      // I know these are binary trees, but this works for k-ary trees
...
function getOffsets(positions) {
  return Object.keys(positions).map(function (i) {
    return +i;
  }).sort(function (a, b) {
    return a - b;
  });
...
function resolveOverlaps(dl) {
  if (dl.length < 2) return dl;
  var domainList = _.cloneDeep(dl);
  domainList.sort(function (a, b) {
    return a.start - b.start;
  });
  var trimmed = [];
  var last = _.clone(domainList[0]);
  var i = 1;
    ...
    if (d.start >= last.end) {
      ...
      if (d.nSeqs > last.nSeqs) {
        if (d.end >= last.end) {
          ...
          last = _.clone(d);
          var j = i + 1;
          while (j < domainList.length && domainList[j].start < _l.start) {
        ...
        if (last.end >= d.end) {
          ...
          _l2.start = last.end;
          var _j = i + 1;
          while (_j < domainList.length && domainList[_j].start < _l2.start) {
            domainList[_j - 1] = domainList[_j];
            _j++;
          }
          domainList[_j - 1] = _l2;
        }
